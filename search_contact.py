...
class SearchContact:
    ...
    def search_contact_by_phone_number(self):
        search_criteria = self.__phone_search_criteria.get()
        ...
        lines = data.split("===================================================================")
        for entry in lines:
            if "Phone Number:" in entry and search_criteria.lower() in entry.lower():
                search_results.append(entry)
    ...
    def go_back_to_search(self):
        ...
        # Add the search bar and button for phone number search
        self.__phone_search_label = Label(self.__contact_frame, text="Search Phone Number", fg="black", bg="#c3e7fd")
        self.__phone_search_label.pack(anchor="w", padx=50, pady=15)
        self.__phone_search_label = Label(self.__contact_frame, text="Enter an 11-digits phone number starting with '09'", fg="black", bg="#c3e7fd")
        self.__phone_search_label.pack(anchor="w", padx=50, pady=15)
        self.__phone_search_entry = Entry(self.__contact_frame, fg="black", width=50, font=("Helvetica", 12), bg="#f3faff", textvariable=self.__phone_search_criteria)
        self.__phone_search_entry.pack(anchor="w", padx=50, pady=5)
        self.__phone_search_button = Button(self.__contact_frame, text="Search", command=self.search_contact_by_phone_number)
        self.__phone_search_button.pack(anchor="w", padx=50, pady=5)
        ...
        # Reset the results_displayed flag
        self.__results_displayed = False

        canvas = self.__contact_frame.master
